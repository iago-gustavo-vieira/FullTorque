...
if (!$tabela_existe) {
    ...
    if ($coluna_existe) {
        ...
        // Remover a coluna mecanico_id da tabela agendamentos
        $sql = "ALTER TABLE agendamentos DROP FOREIGN KEY agendamentos_ibfk_3";
        $conexao->query($sql);
        
        $sql = "ALTER TABLE agendamentos DROP COLUMN mecanico_id";
        if ($conexao->query($sql)) {
            echo "<p>Coluna 'mecanico_id' removida da tabela 'agendamentos' com sucesso!</p>";
        } else {
            echo "<p>Erro ao remover coluna 'mecanico_id': " . $conexao->error . "</p>";
        }
    }
}
